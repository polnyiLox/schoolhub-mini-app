...
describe('date helpers', () => {
  ...
  it('adds days without mutating input', () => {
    ...
    expect(toIsoDate(addDays(date, 2))).toBe('2026-01-03');
    expect(toIsoDate(date)).toBe('2026-01-01');
  });
  it('crosses a month boundary', () =>
    expect(toIsoDate(addDays(new Date(2026, 0, 31), 1))).toBe('2026-02-01'));
  it('formats day in Russian', () =>
    expect(formatDay('2026-09-14')).toContain('14'));
  it('capitalizes Russian weekday', () =>
    expect(formatWeekday('2026-09-14')).toMatch(/^[А-Я]/));
  it('formats aware datetime', () =>
    expect(formatDateTime('2026-09-14T16:30:00+04:00')).toContain('14'));
  ...
  it.each([
    ...
    [14, 'Добрый день'],
    [21, 'Добрый вечер'],
  ] as const)('returns greeting for hour %i', (hour, expected) =>
