...
describe('Telegram bridge', () => {
  ...
  it('returns Telegram WebApp', () => {
    const app = telegram();
    expect(getTelegramWebApp()).toBe(app);
  });
  it('announces readiness and expands', () => {
    const app = telegram();
    initializeTelegram();
    expect(app.ready).toHaveBeenCalledOnce();
    expect(app.expand).toHaveBeenCalledOnce();
  });
  it('enables closing confirmation', () => {
    const app = telegram();
    initializeTelegram();
    expect(app.enableClosingConfirmation).toHaveBeenCalledOnce();
  });
  ...
  it('removes dark theme for light Telegram', () => {
    document.documentElement.classList.add('dark');
    telegram('light');
    initializeTelegram();
    expect(document.documentElement).not.toHaveClass('dark');
  });
  it('sends success haptic', () => {
    const app = telegram();
    hapticSuccess();
    expect(app.HapticFeedback?.notificationOccurred).toHaveBeenCalledWith(
      'success',
    );
  });
  it('sends error haptic', () => {
    const app = telegram();
    hapticError();
    expect(app.HapticFeedback?.notificationOccurred).toHaveBeenCalledWith(
      'error',
